...
import { compilePermissionBundles, permissionDigest } from './permissions.ts'
...
export function compose(
  ...
  // --- browser screens ------------------------------------------------------
  // Browser execution is data in the manifest, not imports in a host route.
  // Collect every registry before validating references so dependency order does
  // not accidentally become part of the public extension contract.
  const browserRef = (m: KetModule, name: string): string =>
    name.includes('.') ? name : qualify(m.name, name)
  const messageRef = (m: KetModule, name: string): string => `${m.name}.${name}`
  const validBrowserType = (spec: unknown): spec is string => typeof spec === 'string' && parseType(spec).ok
  const checkBrowserReference = (
    m: KetModule,
    kind: string,
    from: string,
    ref: string,
    ...
    if (!exists) {
      diag.add({
        code: `E_BROWSER_${kind.toUpperCase()}_MISSING`,
        module: m.name,
        message: `${from} references unknown ${kind} "${ref}"`,
      })
      return
    }
    ...
    if (!canSee(m, owner)) {
      diag.add({
        code: 'E_BROWSER_DEPENDENCY',
        module: m.name,
        message: `${from} reaches ${kind} "${ref}" without depending on "${owner}"`,
      })
    }
  }
  const browserClientPathOk = (path: string): boolean =>
    path.length > 0 &&
    !path.startsWith('/') &&
    !path.includes('\\') &&
    !path.includes('?') &&
    !path.includes('#') &&
    !path.split('/').includes('..')
  const browserWidgetBuiltins = new Set(['text', 'badge', 'code', 'money'])

  for (const m of order) {
    for (const [name, def] of Object.entries(m.browser.widgets ?? {})) {
      const id = qualify(m.name, name)
      if (manifest.browser.widgets[id]) {
        diag.add({
          code: 'E_BROWSER_WIDGET_DUPLICATE',
          module: m.name,
          message: `browser widget "${id}" is declared twice`,
        })
        continue
      }
      const implementations = Number(def.builtin !== undefined) + Number(def.client !== undefined)
      if (implementations !== 1) {
        diag.add({
          code: 'E_BROWSER_WIDGET_IMPLEMENTATION',
          module: m.name,
          message: `browser widget "${id}" needs exactly one builtin or client implementation`,
        })
      }
      if (def.ssrBuiltin !== undefined && def.client === undefined) {
        diag.add({
          code: 'E_BROWSER_WIDGET_FALLBACK',
          module: m.name,
          message: `browser widget "${id}" declares an SSR fallback without a client implementation`,
        })
      }
      for (const [field, value] of [
        ['builtin', def.builtin],
        ['ssrBuiltin', def.ssrBuiltin],
      ] as const) {
        if (value !== undefined && !browserWidgetBuiltins.has(value)) {
          diag.add({
            code: 'E_BROWSER_WIDGET_BUILTIN',
            module: m.name,
            message: `browser widget "${id}" ${field} "${String(value)}" is unsupported`,
          })
        }
      }
      for (const [prop, type] of Object.entries(def.props ?? {})) {
        if (!validBrowserType(type)) {
          diag.add({
            code: 'E_BROWSER_WIDGET_PROP_TYPE',
            module: m.name,
            message: `browser widget "${id}" prop "${prop}" has invalid type "${String(type)}"`,
          })
        }
      }
      if (def.client !== undefined && !m.assets) {
        diag.add({
          code: 'E_BROWSER_WIDGET_WITHOUT_ASSETS',
          module: m.name,
          message: `browser widget "${id}" declares client module "${def.client}" but has no assets directory`,
        })
      }
      if (def.client !== undefined && !browserClientPathOk(def.client)) {
        diag.add({
          code: 'E_BROWSER_WIDGET_CLIENT_PATH',
          module: m.name,
          message: `browser widget "${id}" client path must stay inside its module assets directory`,
        })
      }
      manifest.browser.widgets[id] = {
        id,
        by: m.name,
        props: { ...(def.props ?? {}) },
        ...(def.builtin === undefined ? {} : { builtin: def.builtin }),
        ...(def.ssrBuiltin === undefined ? {} : { ssrBuiltin: def.ssrBuiltin }),
  ...
  for (const m of order) {
    for (const [name, def] of Object.entries(m.browser.resources ?? {})) {
      const id = qualify(m.name, name)
      if (manifest.browser.resources[id]) {
        diag.add({
          code: 'E_BROWSER_RESOURCE_DUPLICATE',
          module: m.name,
          message: `browser resource "${id}" is declared twice`,
        })
        continue
      }
      const source = browserRef(m, def.source)
      const needs = browserRef(m, def.needs)
      checkBrowserReference(
        m,
        'function',
        `browser resource "${id}" source`,
        source,
        !!manifest.functions[source],
      )
      checkBrowserReference(
        m,
        'function',
        `browser resource "${id}" permission`,
        needs,
        !!manifest.functions[needs],
      )
      for (const [field, type] of Object.entries(def.fields ?? {})) {
        if (manifest.functions[source]?.output[field] !== type) {
          diag.add({
            code: 'E_BROWSER_RESOURCE_OUTPUT',
            module: m.name,
            message: `browser resource "${id}" field "${field}" (${type}) must match the output of "${source}"`,
          })
        }
      }
      if (!['primary', 'essential', 'deferred'].includes(def.phase)) {
        diag.add({
          code: 'E_BROWSER_RESOURCE_PHASE',
          module: m.name,
          message: `browser resource "${id}" has unsupported phase "${String(def.phase)}"`,
        })
      }
      if (def.phase !== 'primary') {
        for (const field of Object.keys(manifest.functions[source]?.output ?? {})) {
          if (field in (def.fields ?? {})) continue
          diag.add({
            code: 'E_BROWSER_RESOURCE_OUTPUT',
            module: m.name,
            message: `browser resource "${id}" source "${source}" exposes undeclared field "${field}"`,
            hint: 'a joined browser resource uses the generic function endpoint, so its function output must match the projected fields exactly',
          })
        }
      }
      for (const [field, type] of Object.entries(def.fields ?? {})) {
        if (!validBrowserType(type)) {
          diag.add({
            code: 'E_BROWSER_RESOURCE_FIELD_TYPE',
            module: m.name,
            message: `browser resource "${id}" field "${field}" has invalid type "${String(type)}"`,
          })
        }
      }
      const keyType = def.fields?.[def.key]
      const parsedKey = keyType === undefined ? null : parseType(keyType)
      if (!parsedKey?.ok || parsedKey.optional || parsedKey.base === 'json') {
        diag.add({
          code: 'E_BROWSER_RESOURCE_KEY',
          module: m.name,
          message: `browser resource "${id}" key "${def.key}" must be a declared, required scalar field`,
        })
      }
      if (def.batch) {
        const inputType = manifest.functions[source]?.input[def.batch.input]
        if (inputType !== 'json') {
          diag.add({
            code: 'E_BROWSER_RESOURCE_BATCH_INPUT',
            module: m.name,
            message: `browser resource "${id}" batch input "${def.batch.input}" must be a json input on "${source}"`,
          })
        }
        if (!Number.isInteger(def.batch.max) || def.batch.max < 1 || def.batch.max > 1_000) {
          diag.add({
            code: 'E_BROWSER_RESOURCE_BATCH_MAX',
            module: m.name,
            message: `browser resource "${id}" batch max must be an integer from 1 to 1000`,
          })
        }
      }
      if (
        def.cache &&
        (!['request', 'context'].includes(def.cache.scope) ||
          !Number.isInteger(def.cache.ttlMs) ||
          def.cache.ttlMs < 0 ||
          def.cache.ttlMs > 3_600_000)
      ) {
        diag.add({
          code: 'E_BROWSER_RESOURCE_CACHE',
          module: m.name,
          message: `browser resource "${id}" cache needs scope "request" or "context" and an integer ttlMs from 0 to 3600000`,
        })
      }
      manifest.browser.resources[id] = {
        ...def,
        id,
        by: m.name,
        source,
        needs,
        fields: { ...(def.fields ?? {}) },
        ...(def.batch ? { batch: { ...def.batch } } : {}),
        ...(def.cache ? { cache: { ...def.cache } } : {}),
      }
    }
  }

  const composeBrowserColumn = (
    m: KetModule,
    screenId: string,
    raw: BrowserColumnDef,
  ): ComposedBrowserColumn => {
    const resource = browserRef(m, raw.resource)
    const widget = browserRef(m, raw.widget)
    checkBrowserReference(
      m,
      'resource',
      `browser column "${screenId}.${raw.id}"`,
      resource,
      !!manifest.browser.resources[resource],
    )
    checkBrowserReference(
      m,
      'widget',
      `browser column "${screenId}.${raw.id}"`,
      widget,
      !!manifest.browser.widgets[widget],
    )
    const resourceDef = manifest.browser.resources[resource]
    const widgetDef = manifest.browser.widgets[widget]
    const expectedProps = Object.keys(widgetDef?.props ?? {})
    const boundProps = Object.keys(raw.bind ?? {})
    for (const prop of new Set([...expectedProps, ...boundProps])) {
      const field = raw.bind?.[prop]
      if (!(prop in (widgetDef?.props ?? {}))) {
        diag.add({
          code: 'E_BROWSER_COLUMN_BINDING',
          module: m.name,
          message: `browser column "${screenId}.${raw.id}" binds unknown widget prop "${prop}"`,
        })
        continue
      }
      if (field === undefined || !(field in (resourceDef?.fields ?? {}))) {
        diag.add({
          code: 'E_BROWSER_COLUMN_BINDING',
          module: m.name,
          message: `browser column "${screenId}.${raw.id}" must bind widget prop "${prop}" to a resource field`,
        })
        continue
      }
      if (widgetDef?.props[prop] !== resourceDef?.fields[field]) {
        diag.add({
          code: 'E_BROWSER_COLUMN_BINDING_TYPE',
          module: m.name,
          message: `browser column "${screenId}.${raw.id}" binds ${resource}.${field} (${resourceDef?.fields[field]}) to ${widget}.${prop} (${widgetDef?.props[prop]})`,
        })
      }
    }
    const operations = raw.operations ?? ['display']
    if (
      !operations.length ||
      operations.some((operation) => !['display', 'sort', 'filter'].includes(operation))
    ) {
      diag.add({
        code: 'E_BROWSER_COLUMN_OPERATION',
        module: m.name,
        message: `browser column "${screenId}.${raw.id}" declares an unsupported operation`,
      })
    }
    return {
      ...raw,
  ...
  for (const m of order) {
    for (const [name, def] of Object.entries(m.browser.screens ?? {})) {
      const id = qualify(m.name, name)
      if (manifest.browser.screens[id]) {
        diag.add({
          code: 'E_BROWSER_SCREEN_DUPLICATE',
          module: m.name,
          message: `browser screen "${id}" is declared twice`,
        })
        continue
      }
      if (major(def.contract) === null) {
        diag.add({
          code: 'E_BROWSER_SCREEN_CONTRACT',
          module: m.name,
          message: `browser screen "${id}" contract "${def.contract}" is not a version`,
        })
      }
      if (def.kind !== 'list') {
        diag.add({
          code: 'E_BROWSER_SCREEN_KIND',
          module: m.name,
          message: `browser screen "${id}" has unsupported kind "${String(def.kind)}"`,
        })
      }
      const primary = browserRef(m, def.primary)
      checkBrowserReference(
        m,
        'resource',
        `browser screen "${id}"`,
        primary,
        !!manifest.browser.resources[primary],
      )
      if (manifest.browser.resources[primary]?.phase !== 'primary') {
        diag.add({
          code: 'E_BROWSER_SCREEN_PRIMARY',
          module: m.name,
          message: `browser screen "${id}" primary resource "${primary}" must use phase "primary"`,
        })
      }
      if (!(def.rowKey in (manifest.browser.resources[primary]?.fields ?? {}))) {
        diag.add({
          code: 'E_BROWSER_SCREEN_ROW_KEY',
          module: m.name,
          message: `browser screen "${id}" row key "${def.rowKey}" is absent from "${primary}"`,
        })
      }
      const joints: (typeof manifest.browser.screens)[string]['joints'] = {}
      for (const [joint, jointDef] of Object.entries(def.joints ?? {})) {
        if (jointDef.kind !== 'columns' || major(jointDef.contract) === null) {
          diag.add({
            code: 'E_BROWSER_JOINT_CONTRACT',
            module: m.name,
            message: `browser joint "${id}:${joint}" needs kind "columns" and a versioned contract`,
          })
        }
        joints[joint] = { ...jointDef, owner: m.name }
      }
      manifest.browser.screens[id] = {
        ...def,
        id,
        by: m.name,
        primary,
        title: messageRef(m, def.title),
        ...(def.description === undefined ? {} : { description: messageRef(m, def.description) }),
        columns: def.columns.map((column) => composeBrowserColumn(m, id, column)),
        joints,
      }
    }
  }

  const filledJoints = new Map<string, string[]>()
  for (const m of order) {
    for (const [jointRef, fill] of Object.entries(m.browser.fills ?? {})) {
      const colon = jointRef.lastIndexOf(':')
      const rawScreen = colon < 0 ? jointRef : jointRef.slice(0, colon)
      const jointName = colon < 0 ? '' : jointRef.slice(colon + 1)
      const screenId = browserRef(m, rawScreen)
      const canonicalJoint = `${screenId}:${jointName}`
      const screen = manifest.browser.screens[screenId]
      if (!screen || !jointName || !screen.joints[jointName]) {
        diag.add({
          code: 'E_BROWSER_FILL_JOINT_MISSING',
          module: m.name,
          message: `browser fill "${jointRef}" targets an unknown screen joint`,
        })
        continue
      }
      if (!canSee(m, screen.by)) {
        diag.add({
          code: 'E_BROWSER_DEPENDENCY',
          module: m.name,
          message: `browser fill "${jointRef}" reaches "${screen.by}" without depending on it`,
        })
      }
      const expectedMajor = major(screen.joints[jointName]!.contract)
      if (major(fill.compatible) === null || major(fill.compatible) !== expectedMajor) {
        diag.add({
          code: 'E_BROWSER_FILL_INCOMPATIBLE',
          module: m.name,
          message: `browser fill "${jointRef}" requires "${fill.compatible}" but the joint provides "${screen.joints[jointName]!.contract}"`,
        })
      }
      const contributors = filledJoints.get(canonicalJoint) ?? []
      if (!screen.joints[jointName]!.multiple && contributors.length) {
        diag.add({
          code: 'E_BROWSER_FILL_CARDINALITY',
          module: m.name,
          message: `browser joint "${canonicalJoint}" accepts only one fill (already filled by "${contributors[0]}")`,
        })
      }
      contributors.push(m.name)
      filledJoints.set(canonicalJoint, contributors)
      const columns = fill.columns.map((column) => composeBrowserColumn(m, screenId, column))
      screen.columns.push(...columns)
      manifest.browser.fills.push({
        joint: canonicalJoint,
        by: m.name,
        compatible: fill.compatible,
        columns: columns.map((column) => column.id),
      })
    }
  }

  for (const screen of Object.values(manifest.browser.screens)) {
    const seen = new Map<string, string>()
    for (const column of screen.columns) {
      if (!/^[a-z][a-z0-9_.-]*$/.test(column.id)) {
        diag.add({
          code: 'E_BROWSER_COLUMN_ID',
          module: column.by,
          message: `browser screen "${screen.id}" has invalid column id "${column.id}"`,
        })
      }
      const previous = seen.get(column.id)
      if (previous) {
        diag.add({
          code: 'E_BROWSER_COLUMN_DUPLICATE',
          module: column.by,
          message: `browser screen "${screen.id}" column "${column.id}" is already contributed by "${previous}"`,
        })
      } else seen.set(column.id, column.by)
    }
  }

  const browserFingerprint = {
    version: manifest.browser.version,
    widgets: Object.values(manifest.browser.widgets)
      .map(({ id, by, props, builtin, ssrBuiltin, client }) => ({
        id,
        by,
        props,
        builtin,
    ...
    resources: Object.values(manifest.browser.resources)
      .map(({ id, by, source, needs, phase, key, fields, batch, cache }) => ({
        id,
        by,
        source,
        needs,
      ...
      .sort((a, b) => a.id.localeCompare(b.id)),
    screens: Object.values(manifest.browser.screens)
      .map(({ id, by, kind, contract, primary, rowKey, title, description, columns, joints }) => ({
        id,
        by,
        kind,
        contract,
  ...
  manifest.browser.revision = permissionDigest(browserFingerprint).slice(0, 16)
