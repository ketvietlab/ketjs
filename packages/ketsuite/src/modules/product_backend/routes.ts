...
export const routes: Record<string, RouteEntry> = {
  '/admin/products': (ctx: ServeContext): Route => async (url, req) => {
    ...
    const filter = { search }
    const rows = (await ctx.call('product.listTemplates', {
      ...filter, withVariants: true, limit: PAGE_SIZE, offset: (current - 1) * PAGE_SIZE,
    }, url, req)) as Array<{
        ...
        body: productsScreen(_, rows.map((r): TemplateRow => ({
          ...
          chrome: {
            ...
            search: {
              ...
              keep: view === 'list' ? {} : { view },
              facets: search ? [{ label: `${_('backend.chrome.searchFacet')}: ${search}`, without: withParam(url, 'q', null) }] : [],
            },
