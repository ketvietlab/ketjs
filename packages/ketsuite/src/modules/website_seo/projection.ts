...
/**
 * Namespaces the deployment owns even when no module claims them by prefix.
 *
 * The authoritative list is the composed manifest — see `reservedPrefixes()` —
 * because a hardcoded list drifts from the routes that actually answer. These
 * two are the exceptions the manifest cannot supply: `/api` and `/internal/v1`
 * are reserved as families rather than registered as single paths.
 */
const ALWAYS_RESERVED = ['/api', '/internal/v1'] as const

/**
 * The public path namespaces a sitemap must never list and robots must always
 * disallow, derived from what the deployment actually serves.
 *
 ...
 * excluding it from the sitemap would quietly delist real content.
 */
