...
const upload =
  ...
  async (url, req) => {
    ...
    try {
      ...
      for await (const part of multipart(req, type, { maxBytes: ctx.config.uploadMax })) {
        if (part.filename !== undefined) {
          ...
          uploadPart = {
            ...
            size: stored.size,
            checksum: stored.etag as string,
          }
...
const download =
  ...
  async (url, req, params) => {
    ...
    const publicAttachment = (await ctx.call(
      'storage.getPublicAttachment',
      { id: params.id },
      url,
      req,
    )) as Attachment | null
    ...
    if (attachment.kind === 'url' && attachment.url)
      return withHeaders(text('', { status: 302 }), { location: attachment.url })
    ...
    const headers = {
      ...
      'cache-control': attachment.public ? 'public, max-age=3600' : 'private, no-store',
    }
    if (req.method === 'HEAD') {
    ...
    if (showInline && attachment.size >= 1024 * 1024) {
      ...
      if (signed) return withHeaders(text('', { status: 302 }), { location: signed })
    }
