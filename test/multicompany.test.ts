...
import { bootApp, callFn, compose, defineApp, defineModule, from, migrateOne, registerFunctions, sqliteAdapter, json } from 'ketjs'
...
const books = defineModule({
  ...
  functions: {
    add: { input: { id: 'id', memo: 'text' }, effects: ['write:books.Entry'], handler: (ctx: Ctx, a) => ctx.db.insert('books.Entry', a) },
    list: { effects: ['read:books.Entry'], handler: (ctx: Ctx) => ctx.db.all(from(ctx.table('books.Entry'))) },
    listPlain: { effects: ['read:books.Entry'], handler: (ctx: Ctx) => ctx.db.select('books.Entry') },
    everywhere: { effects: ['read:books.Entry'], crossCompany: true, handler: (ctx: Ctx) => ctx.db.all(from(ctx.table('books.Entry'))) },
  },
})
...
test('multi-company: a read spans the set, a write lands in one', async () => {
  ...
  assert.deepEqual(rows.map(r => `${String(r.id)}/${String(r.companyId)}`), ['a1/acme', 'g1/globex', 'i1/initech'],
...
test('multi-company: the convenience select spans the set too, not just the query builder', async () => {
  ...
  await callFn('books.add', { id: 'a1', memo: 'x' }, { adapter, manifest, scope: { company: 'acme' } })
  await callFn('books.add', { id: 'g1', memo: 'y' }, { adapter, manifest, scope: { company: 'globex' } })
  const seen = await callFn('books.listPlain', {}, { adapter, manifest, scope: { company: 'acme', companies: ['acme', 'globex'] } })
...
test('multi-company: writing somewhere you cannot read back is refused before the query runs', async () => {
  ...
  await assert.rejects(
    () => callFn('books.add', { id: 'x', memo: 'x' }, { adapter, manifest, scope: { company: 'initech', companies: ['acme', 'globex'] } }),
    (e: unknown) => {
...
test('multi-company: a readable set with nothing to write to says which companies it has', async () => {
  const { adapter, manifest } = await boot()
  await assert.rejects(
    () => callFn('books.add', { id: 'x', memo: 'x' }, { adapter, manifest, scope: { company: null, companies: ['acme', 'globex'] } }),
    (e: unknown) => {
...
test('multi-company: naming no company at all is still refused, not answered', async () => {
  const { adapter, manifest } = await boot()
  await assert.rejects(() => callFn('books.list', {}, { adapter, manifest, scope: { company: null } }),
    (e: unknown) => { assert.equal((e as { code: string }).code, 'E_NO_COMPANY_IN_SCOPE'); return true })
  await adapter.close()
})
...
test('multi-company: crossCompany still means all of them, and still has to be declared', async () => {
  ...
  const all = await callFn('books.everywhere', {}, { adapter, manifest, scope: { company: 'acme', companies: ['acme'] } })
...
const app = defineApp({
  name: 'booksapp', modules: [books], headless: true,
  serve: {
    bootstrap: ['books'],
