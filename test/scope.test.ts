...
const ledger = defineModule({
  ...
  functions: {
    ...
    forge: defineFn({
      ...
      effects: ['write:ledger.Invoice'],
      handler: async (ctx: Ctx, a) => ctx.db.insert('ledger.Invoice', { id: a.id, total: 1, companyId: a.companyId } as Row),
    }),
    currencies: defineFn({
      input: {},
...
test('scope: a model that does not declare one is a build error', () => {
  ...
  const e = (() => { try { compose([sloppy], { headless: true }) } catch (err) { return err as KetError } })()!
...
test('scope: the columns are added by the composer, not by the module', () => {
  ...
  assert.equal('companyId' in manifest.models['ledger.Currency']!.fields, false, 'shared data carries no company')
})

test('scope: a module cannot extend the scope columns', () => {
  const meddler = defineModule({ name: 'meddler', depends: ['ledger'], extend: { 'ledger.Invoice': { companyId: 'text?' } } })
...
test('scope: one company cannot read another, even asking for everything', async () => {
  ...
  assert.deepEqual(seenByA.map(r => r.id), ['a1'])
  assert.deepEqual(seenByB.map(r => r.id), ['b1'])
  assert.equal((await db.all('SELECT * FROM ledger_invoice', [])).length, 2, 'both rows are in the same table')
  await db.close()
})
...
test('scope: a write is stamped from the request, and cannot be aimed elsewhere', async () => {
  ...
  await assert.rejects(
    ...
    (e: unknown) => {
      ...
      assert.match((e as { hint: string }).hint, /the scope columns come from the request, not from the caller/)
      return true
    })
...
test('scope: shared data needs no company, and is visible to all of them', async () => {
  ...
  for (const scope of [A, B, { company: null }]) {
    ...
    assert.deepEqual(rows.map(r => r.code), ['VND'])
  }
  await db.close()
})
...
test('scope: crossCompany is the only way across, and it is declared', async () => {
  ...
  const all = (await callFn('ledger.listAllCompanies', {}, { adapter: db, manifest, scope: A })).value as Row[]
...
test('scope: branch narrows within a company, and is not a permission', async () => {
  const db = await boot()
  await callFn('ledger.add', { id: 'h1', total: 1 }, { adapter: db, manifest, scope: { company: 'acme', branches: ['hanoi'] } })
  await callFn('ledger.add', { id: 's1', total: 2 }, { adapter: db, manifest, scope: { company: 'acme', branches: ['saigon'] } })

  const hanoi = (await callFn('ledger.list', {}, { adapter: db, manifest, scope: { company: 'acme', branches: ['hanoi'] } })).value as Row[]
  assert.deepEqual(hanoi.map(r => r.id), ['h1'])

  const both = (await callFn('ledger.list', {}, { adapter: db, manifest, scope: { company: 'acme', branches: ['hanoi', 'saigon'] } })).value as Row[]
  ...
  const risky = defineModule({
    ...
    functions: {
      pair: defineFn({
        input: {},
        effects: ['write:ledger.Invoice'],
        handler: async (ctx: Ctx) => ctx.tx(async (tx) => {
    ...
    functions: {
      pair: defineFn({
        ...
        handler: async (ctx: Ctx) => ctx.tx(async (tx) => {
          ...
          return (await tx.db.all(from(tx.table('ledger.Invoice')).where(eq(tx.table('ledger.Invoice').id, 'q1')))).length
        }),
      }),
    },
  })
