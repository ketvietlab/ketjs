...
const violations = (fn: () => unknown): Array<{ code: string; message: string }> => {
  try { fn() } catch (e) { return (e as { items?: Array<{ code: string; message: string }> }).items ?? [] }
...
const sales = defineModule({
  ...
  menus: {
    ...
    'sales.quotes': { parent: 'sales.orders', label: 'menu.quotes', path: '/quotes', needs: 'sales.listQuotes', sequence: 5 },
  },
  messages: { vi: { 'menu.app': 'Bán hàng', 'menu.orders': 'Đơn hàng', 'menu.list': 'Đơn hàng', 'menu.quotes': 'Báo giá' } },
})

const admin = defineModule({
  name: 'admin',
  menus: { admin: { label: 'menu.app', sequence: 90 }, 'admin.settings': { parent: 'admin', label: 'menu.settings', path: '/admin' } },
})
...
test('menu: a module declares its entries and composition gathers them', () => {
  ...
  assert.deepEqual(tree.map(n => n.id), ['sales', 'admin'], 'sequence orders the apps')
  assert.deepEqual(tree[0]!.children[0]!.children.map(n => n.id), ['sales.quotes', 'sales.list'],
...
test('menu: hanging an entry under another module needs the dependency', () => {
  const rogue = defineModule({ name: 'rogue', menus: { 'rogue.x': { parent: 'sales', label: 'x', path: '/x' } } })
...
test('menu: a parent nobody declared is refused at composition, not at render', () => {
  const orphan = defineModule({ name: 'orphan', menus: { 'orphan.x': { parent: 'nowhere', label: 'x', path: '/x' } } })
...
test('menu: what the viewer may not call, the viewer does not see', () => {
  ...
  assert.deepEqual(tree[0]!.children[0]!.children.map(n => n.id), ['sales.quotes'],
    'the entry behind a function they cannot call is gone')

  const none = buildMenu(m, { allow: [] })
  assert.deepEqual(none.map(n => n.id), ['admin'],
...
test('menu: uninstalling a module takes its entries with it', () => {
  ...
  assert.deepEqual(buildMenu(live).map(n => n.id), ['admin'])
})
...
test('menu: a gate on a module this build does not ship hides the entry, quietly', () => {
  ...
  const gated = defineModule({
    name: 'gated',
    menus: { gated: { label: 'x' }, 'gated.pages': { parent: 'gated', label: 'p', path: '/p', needs: 'website.listPages' } },
  })
  const m = compose([gated])
  assert.deepEqual(buildMenu(m).map(n => n.id), [], 'nothing to lead to, so nothing shown')
})

test('menu: a gate on a module that is here, naming a function that is not, is a typo', () => {
  const typo = defineModule({
    name: 'typo', depends: ['sales'],
