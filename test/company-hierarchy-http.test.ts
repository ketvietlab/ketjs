...
test('company hierarchy HTTP preserves nesting, archived state, encoding and locale', async (t: TestContext) => {
  ...
  for (const [id, name, parentId] of [
    ['root', 'Root Company', null], ['child/a', 'Child Company', 'root'], ['grandchild', 'Grandchild', 'child/a'],
  ] as const) {
    await fixture('partner.savePartner', { id: `${id}:party`, kind: 'company', name })
    await fixture('company.saveCompany', { id, code: id, partnerId: `${id}:party`, parentId, currency: 'VND' })
  }
  ...
  assert.equal((await app.client.request('/admin/companies/hierarchy?lang=en', { method: 'POST' })).status, 405)
})
