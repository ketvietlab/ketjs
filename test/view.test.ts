...
test('signals: effects track what they read, and a no-op write costs nothing', () => {
  ...
  assert.equal(runs, 2)
  a.set(2)
  assert.equal(runs, 2, 'writing the same value must not re-run')
  b.set(99)
  assert.equal(runs, 2, 'an untracked signal must not re-run it')
})
...
const view = (items: Item[]) =>
  html`<ul>${each(
    items,
    (it) => (it as Item).id,
    (it) => html`<li data-id=${(it as Item).id}>${(it as Item).name}</li>`,
  )}</ul>`
...
test('view: re-rendering unchanged data touches nothing', () => {
  ...
  root.render(view(items))
  assert.equal(
    Object.values(host.ops).reduce((a, b) => a + b, 0),
    0,
  )
})
...
test('view: rendered markup is correct, attributes and all', () => {
  ...
  root.render(
    html`<div class="box" data-n=${3}><span>${'xin chào'}</span>${when(true, () => html`<b>có</b>`)}</div>`,
  )
  assert.equal(
    host.html(container.children![0]!),
    '<div class="box" data-n="3"><span>xin chào</span><b>có</b></div>',
  )
})
