...
import { callFn, compose, defineModule, from, migrateOne, registerFunctions, sqliteAdapter, project } from 'ketjs'
...
test('project: a list is projected element by element', () => {
  const got = project('f', { id: 'id' }, [{ id: '1', secret: 'x' }, { id: '2', secret: 'y' }])
...
test('project: an optional field may be absent, which is what a result union needs', () => {
  ...
  assert.deepEqual(project('f', { ok: 'bool', qty: 'float?', errors: 'json?' }, { ok: true, qty: 3 }), { ok: true, qty: 3 })
  assert.deepEqual(project('f', { ok: 'bool', qty: 'float?', errors: 'json?' }, { ok: false, errors: [] }), { ok: false, errors: [] })
})

test('project: a required field the handler forgot is named, with the fix in the hint', () => {
  assert.throws(() => project('shop.list', { id: 'id', productName: 'text' }, { id: '1', product: { name: 'Xoài' } }),
    (e: unknown) => {
...
test('project: it is one level deep, and that is a limit rather than an oversight', () => {
  ...
  assert.deepEqual(got, { id: '1', product: { name: 'Xoài', cost: 12000 } },
...
test('projection: the cost does not leave, and the scope column does not either', async () => {
  ...
  await adapter.run('INSERT INTO shop_product (id,name,cost,price) VALUES (?,?,?,?)', ['p1', 'Xoài', '12000', '30000'])
  await adapter.run('INSERT INTO shop_order (id,"companyId","productId",qty) VALUES (?,?,?,?)', ['o1', 'acme', 'p1', 5])
