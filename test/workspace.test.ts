...
test('umbrella: one codebase composes several apps from overlapping modules', () => {
  ...
  assert.ok(!('checkout.placeOrder' in ws.apps.storefront!.functions), 'storefront must not expose admin-only functions')
...
test('umbrella: apps sharing a datastore get one union schema', () => {
  ...
  assert.ok('checkout_order' in main.schema.tables, 'a table only one app knows about still lives in the shared store')
...
test('umbrella: a headless app renders nothing and may not install a theme', () => {
  ...
  assert.throws(() => defineApp({ name: 'x', modules: [catalog], theme, headless: true }), /headless but installs a theme/)
})
