...
const shop = defineModule({
  ...
  functions: {
    list: {
      effects: ['read:shop.Item'],
      output: { id: 'id', name: 'text' },
      handler: (ctx: Ctx) => ctx.db.all(from(ctx.table('shop.Item'))),
    },
    add: {
      input: { id: 'id', name: 'text' },
      effects: ['write:shop.Item'],
      output: { ok: 'bool' },
      handler: async (ctx: Ctx, a) => {
        await ctx.db.insert('shop.Item', a)
        return { ok: true }
      },
    },
  },
...
test('allow: and nothing else, however well-formed the call is', async () => {
  ...
  await assert.rejects(
    () => callFn('shop.add', { id: 'i1', name: 'X' }, { ...o, allow: ['shop.list'] }),
    (e: unknown) => {
      assert.equal((e as { code: string }).code, 'E_FN_NOT_PERMITTED')
      return true
    },
  )
...
test('allow: refusal comes before input validation, so a refused caller learns nothing else', async () => {
  ...
  // may not touch it.
  await assert.rejects(
    () => callFn('shop.add', { nonsense: 1 }, { ...o, allow: [] }),
    (e: unknown) => {
      assert.equal((e as { code: string }).code, 'E_FN_NOT_PERMITTED')
      return true
    },
...
const setup = async () => {
  ...
  for (const [id, login, superuser] of [
    ['u1', 'root', true],
    ['u2', 'sale', false],
  ] as const) {
...
test('roles: a granted function works and an ungranted one does not, over HTTP', async () => {
  ...
  assert.equal(
    await call(sale, 'partner.savePartner', { id: 'p9', kind: 'person', name: 'x' }),
    'E_FN_NOT_PERMITTED',
  )
  assert.equal(
    await call(sale, 'user.saveRole', { id: 'r9', name: 'x' }),
    'E_FN_NOT_PERMITTED',
...
test('roles: revoking takes effect on the next call, not on the next login', async () => {
  ...
  assert.equal(
    await call(sale, 'partner.listPartners'),
    'E_FN_NOT_PERMITTED',
...
test('roles: `permitted` answers with the same list the server enforces', async () => {
  ...
  await run('user.grantFunction', { id: 'g1', roleId: 'r1', fnKey: 'partner.listPartners' })
  await run('user.assignRole', { id: 'a1', userId: 'u2', roleId: 'r1' })
  assert.deepEqual(await run('user.permitted', { userId: 'u2' }), {
    userId: 'u2',
    functions: ['partner.listPartners'],
    superuser: false,
  })
