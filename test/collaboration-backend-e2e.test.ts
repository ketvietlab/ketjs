...
test('Chatter backend E2E: Partner bridge renders and persists its timeline', async (t) => {
  const { e2e, call } = await boot(t)

  const partnerPage = await e2e.client.get('/admin/partner/partners/acme-party?lang=en', {
    headers: { accept: 'text/html' },
  })
  assert.equal(partnerPage.status, 200)
  const html = await partnerPage.text()
  assert.match(html, /data-ui="form-page-aside"/)
  assert.match(html, /data-island="mail\.chatter"/)
  assert.match(html, /&quot;resModel&quot;:&quot;partner\.Partner&quot;/)
  assert.doesNotMatch(html, /Quick information/)

  const client = await e2e.client.get('/_ket/asset/mail_backend/mail-bundle.mjs')
  assert.equal(client.status, 200)
  const clientSource = await client.text()
  assert.match(clientSource, /Generated by tools\/build-backend-client\.mjs — do not edit\./)
  assert.match(clientSource, /partner_mail_backend/)
  assert.doesNotMatch(clientSource, /\.\/mail-view\.mjs/)

  await call('partner_mail_backend.follow', { targetId: 'acme-party' })
  await call('partner_mail_backend.post', {
    id: 'partner-message-1',
    targetId: 'acme-party',
    kind: 'note',
    body: 'Partner handoff confirmed',
  })
  const timeline = await call<{ total: number; messages: Row[] }>('partner_mail_backend.timeline', {
    targetId: 'acme-party',
  })
  assert.equal(timeline.value.total, 1)
  assert.equal(timeline.value.messages[0]?.body, 'Partner handoff confirmed')
})
...
test('Chatter backend E2E: Product bridge renders, follows, posts attachments and updates inbox', async (t) => {
  ...
  assert.match(await bootstrap.text(), /\/_ket\/asset\/mail_backend\/v[0-9a-f]{8}\/mail-bundle\.mjs/)
...
test('Product Variant form keeps its own Chatter, activities and partial-save workspace', async (t) => {
  ...
  assert.match(html, /data-ui="form-page"[^>]*data-scope="product-variant-form-page"/)
  assert.match(html, /data-ui="form-page-aside"/)
  assert.match(html, /data-island="mail\.chatter"/)
