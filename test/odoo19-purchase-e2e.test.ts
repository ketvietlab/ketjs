...
async function bootPurchase(t: TestContext) {
  ...
  await fixture('user.createUser', { id: 'admin', login: 'admin', password: 'correct horse', name: 'Administrator', defaultCompanyId: 'acme', superuser: true })
  ...
  await call('product.saveTemplate', { id: 'goods', name: 'Bàn làm việc', type: 'goods', uomId: 'unit', listPrice: '2000000', purchaseOk: true })
  await call('product.saveVariant', { id: 'desk', templateId: 'goods', defaultCode: 'BAN-01', combinationKey: '' })
  ...
  await call('stock.savePickingType', { id: 'incoming', name: 'Nhập hàng', code: 'incoming', defaultLocationSrcId: 'supplier', defaultLocationDestId: 'stock' })
  for (const [id, code, name, accountType] of [['expense', '6421', 'Chi phí mua hàng', 'expense'], ['payable', '331', 'Phải trả nhà cung cấp', 'liability_payable'], ['tax', '1331', 'Thuế GTGT được khấu trừ', 'asset_current']])
    await call('account.saveAccount', { id, code, name, accountType })
  await call('account.saveJournal', { id: 'purchase-journal', name: 'Mua hàng', code: 'PUR', type: 'purchase' })
  await call('account.saveTax', { id: 'vat10', name: 'GTGT 10%', typeTaxUse: 'purchase', amountType: 'percent', amount: '10' })
  return { e2e, call }
}

test('e2e purchase 19: RFQ to receipt and vendor bill crosses real HTTP', async (t) => {
  const { e2e, call } = await bootPurchase(t)
  await call('purchase.saveSupplierInfo', { id: 'vendor:desk', partnerId: 'vendor', productTemplateId: 'goods', productId: 'desk', productUomId: 'unit', minQty: '1', price: '1500000', discount: '5', delay: 2 })
  const created = (await call<Row>('purchase.createOrder', { id: 'po-1', partnerId: 'vendor', partnerRef: 'NCC/2026/01', pickingTypeId: 'incoming', dateOrder: '2026-08-20T00:00:00.000Z' })).value
  assert.equal(created.name, 'PO00001')
  await call('purchase.addLine', { id: 'po-1:line', orderId: 'po-1', productId: 'desk', productQty: '2', productUomId: 'unit', taxId: 'vat10' })
  ...
  await call('stock.saveMoveLine', { id: 'receipt-line', moveId: 'po-1:line:receipt', quantity: '2', picked: true })
  await call('stock.completePicking', { id: 'po-1:receipt' })
  await call('purchase.syncReceipts', { id: 'po-1' })
  const bill = (await call<Row>('purchase.createVendorBill', { id: 'bill-1', orderId: 'po-1', journalId: 'purchase-journal', expenseAccountId: 'expense', payableAccountId: 'payable', taxAccountId: 'tax' })).value
  assert.equal(bill.amountTotal, '3135000')
  assert.equal(((await call<Row>('purchase.getOrder', { id: 'po-1' })).value).invoiceStatus, 'invoiced')

  await call('purchase.createOrder', { id: 'ui-rfq', partnerId: 'vendor', partnerRef: 'UI/RFQ', pickingTypeId: 'incoming' })
  await call('purchase.addLine', { id: 'ui-rfq:line', orderId: 'ui-rfq', productId: 'desk', productQty: '1', productUomId: 'unit' })
  ...
  const english = await e2e.client.get('/admin/purchase/orders/po-1?lang=en', { headers: { accept: 'text/html' } })
