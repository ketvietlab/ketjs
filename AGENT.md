...
- Pull requests into `develop` carry no CI apart from `permission coverage`. Before pushing, run the tests for the part you changed — including its live-Postgres tests when it has them — plus the quality contracts in `verify.yml` (format, lint, terminology, build, zero-dep, UI, staff-channel, Channel API, doc snippets, types), and list the commands and results in the pull request description. Re-run them after review changes or a rebase.
- Do not run the full test suite as part of routine local development. The full suite runs in CI on the release pull request into `master`.
